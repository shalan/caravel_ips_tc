...
# Input delays
set_input_delay -max 6   -clock [get_clocks {clk}] [all_inputs]
set_input_delay -min 0.50   -clock [get_clocks {clk}] [all_inputs]
set_input_delay -min 1.90   -clock [get_clocks {clk}] [get_ports {wb_rst_i}]

# Input Transition
set_input_transition -min 0.05 [all_inputs]
set_input_transition -max 1.05 [all_inputs]

# Output delays
set_output_delay -max 8.00   -clock [get_clocks {clk}] [all_outputs]
set_output_delay -min 0.50   -clock [get_clocks {clk}] [all_outputs]

# Output loads
set_load 0.14  [all_outputs]
set_load 0.34  [get_ports {irq[*]}]